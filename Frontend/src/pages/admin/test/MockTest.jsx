...
const MockTestPage = () => {
  ...
  const fetchMockTests = async () => {
    try {
      const response = await axios.get(ADMINENDPOINTS.GETMOCKTEST, {
      ...
      if (response.status === 200) {
        ...
        toast.success('Mock tests fetched successfully!');
      }
    } catch (error) {
      console.error('Error fetching mock tests:', error);
      setErrorMessage('Error fetching mock tests, please try again.');
    }
  };
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (response.status === 200) {
          ...
          fetchMockTests();  // Fetch updated tests after deletion
        }
      } catch (error) {
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        fetchMockTests(); // Fetch updated tests after editing
  ...
  const handleSubjectChange = (event) => {
    ...
    if (subject) {
      setFilteredTests(mockTests.filter((test) => test.subject === subject));
    } else {
